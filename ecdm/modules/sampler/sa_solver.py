# copied from https://github.com/scxue/SA-Solver
import torch
...
class SASolver:
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N, order, device):
        ...
        if skip_type == "logSNR":
            ...
            logSNR_steps = lambda_T + torch.linspace(
                torch.tensor(0.0).cpu().item(),
                (lambda_0 - lambda_T).cpu().item() ** (1.0 / order),
                N + 1,
            ).pow(order).to(device)
            ...
            sigma_steps = (
                torch.linspace(sigma_max ** (1.0 / 7), sigma_min ** (1.0 / 7), N + 1)
                .pow(7)
                .to(device)
            )
    ...
    def adams_bashforth_update(
        self, order, x, tau, model_prev_list, t_prev_list, noise, t
    ):
        """
    ...
    def adams_bashforth_update_few_steps(
        self, order, x, tau, model_prev_list, t_prev_list, noise, t
    ):
        """
            ...
            if (
                ...
                gradient_coefficients[0] += (
                    ...
                    * (
                    ...
                    / (
                        ns.marginal_lambda(t_prev_list[-1])
                        - ns.marginal_lambda(t_prev_list[-2])
                    )
                )
    ...
    def adams_moulton_update_few_steps(
        self, order, x, tau, model_prev_list, t_prev_list, noise, t
    ):
        """
        SA-Corrector, without the "rescaling" trick in Appendix D in SA-Solver paper https://arxiv.org/pdf/2309.05019.pdf
    ...
    def sample_few_steps(
        ...
        with torch.no_grad():
            ...
            for step in tqdm(
                range(max(predictor_order, corrector_order - 1), steps + 1)
            ):
    ...
    def sample(
        ...
        assert mode in [
            "few_steps",
            "more_steps",
        ], "mode must be either 'few_steps' or 'more_steps'"
        if mode == "few_steps":
